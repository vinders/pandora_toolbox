if(NOT DEFINED _CWORK_OPTIONS_FOUND)
    ...
    if(IOS OR ANDROID)
        set(CWORK_VIDEO_OPENGL4 OFF CACHE STRING "enable OpenGL 4 features")
        set(_VIDEO_OPENGL4_NOT_SUPPORTED ON CACHE STRING "")
    else()
        if(NOT DEFINED CWORK_VIDEO_OPENGL4)
            option(CWORK_VIDEO_OPENGL4 "enable OpenGL 4 features" ON)
        endif()
    ...
    if(APPLE AND NOT IOS)
        set(CWORK_VIDEO_OPENGLES3 OFF CACHE STRING "enable OpenGLES 3 features")
        set(_VIDEO_OPENGLES3_NOT_SUPPORTED ON CACHE STRING "")
    else()
        if(NOT DEFINED CWORK_VIDEO_OPENGLES3)
            if(IOS OR ANDROID)
                option(CWORK_VIDEO_OPENGLES3 "enable OpenGLES 3 features" ON)
            else()
                option(CWORK_VIDEO_OPENGLES3 "enable OpenGLES 3 features" OFF)
            endif()
        endif()
        if(NOT DEFINED CWORK_OPENGLES3_VERSION)
            set(CWORK_OPENGLES3_VERSION "32" CACHE STRING "maximum OpenGLES 3 revision support")
    ...
    if((NOT WIN32 AND NOT WIN64 AND NOT _WIN32 AND NOT _WIN64) OR NOT MSVC)
        ...
        if(NOT DEFINED CWORK_D3D11_VERSION)
            if(CWORK_WINDOWS_VERSION AND (CWORK_WINDOWS_VERSION STREQUAL "6" OR CWORK_WINDOWS_VERSION STREQUAL "7"))
                set(_DEFAULT_D3D11_MAX_VERSION "111")
            else()
    ...
    macro(cwork_print_options)
        ...
        if(WIN32 OR WIN64 OR _WIN32 OR _WIN64 OR CMAKE_SYSTEM_NAME STREQUAL "Windows")
            ...
            elseif(CWORK_WINDOWS_VERSION STREQUAL "6")
                message("> Minimum Windows: Vista SP1 (2008)")
            else()
        ...
        if(NOT DEFINED _VIDEO_OPENGLES3_NOT_SUPPORTED)
            if(CWORK_VIDEO_OPENGLES3)
                string(SUBSTRING ${CWORK_OPENGLES3_VERSION} 1 1 __CWORK_OPENGLES3_VERSION_MINOR)
                message("> OpenGLES: 3.${__CWORK_OPENGLES3_VERSION_MINOR}")
                unset(__CWORK_OPENGLES3_VERSION_MINOR)
            else()
                message("> OpenGLES: OFF")
            endif()
        endif()
