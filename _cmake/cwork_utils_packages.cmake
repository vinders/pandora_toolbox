if(NOT DEFINED CWORK_UTILS_PACKAGES_FOUND)
            ...
            if(CWORK_AUTODETECTED_FILES)
                foreach(_file IN ITEMS ${CWORK_AUTODETECTED_FILES})
                    ...
                    if (__RELATIVE_RES_PATH_LIST)
                        ...
                        file(COPY "${_file}" DESTINATION "${_RES_OUTPUT_DIR}")
                    endif()
    ...
    #brief:  Create a command to copy resources to build directory after each build
    #params: - target_name: name of the project for which resources are copied (after each build)
    #        - output_dir: build directory
    #        - ARGN: list of source directories
    function(cwork_copy_resource_files_on_build target_name output_dir)
        if(ANDROID)
            set(_RES_OUTPUT_DIR ${output_dir}/app/src/main/res)
        else()
            set(_RES_OUTPUT_DIR ${output_dir})
        ...
        foreach(_dir IN ITEMS ${ARGN})
            ...
            if(CWORK_AUTODETECTED_FILES)
                foreach(_file IN ITEMS ${CWORK_AUTODETECTED_FILES})
                    ...
                    if (__RELATIVE_RES_PATH_LIST)
                        string(REPLACE ";" "/" __RELATIVE_RES_DIR ${__RELATIVE_RES_PATH_LIST})
                        set(_TMP_COMMANDS ${_TMP_COMMANDS}
                            COMMAND ${CMAKE_COMMAND} -E copy_if_different "${_file}" "${_RES_OUTPUT_DIR}/${__RELATIVE_RES_DIR}")
                        unset(__RELATIVE_RES_DIR)
                    else()
                        set(_TMP_COMMANDS ${_TMP_COMMANDS}
                            COMMAND ${CMAKE_COMMAND} -E copy_if_different "${_file}" "${_RES_OUTPUT_DIR}")
                    endif()
                    unset(__RELATIVE_RES_PATH)
                    unset(__RELATIVE_RES_PATH_LIST)
                endforeach()
        ...
        if (_TMP_COMMANDS)
            add_custom_command(TARGET ${target_name} POST_BUILD ${_TMP_COMMANDS} VERBATIM)
        endif()
        unset(_TMP_COMMANDS)
    endfunction()
