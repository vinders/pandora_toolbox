if(NOT DEFINED CWORK_SUBPROJECTS_FOUND)
    ...
    macro(cwork_include_internal_libs)
        if(CWORK_INTERNAL_LIBS)
            ...
            foreach(_internal_lib IN ITEMS ${CWORK_INTERNAL_LIBS})
                ...
                # resources (on Windows, added to each project sources, unless header-only project)
                if(${_TMP_LIB_NAME}_INTERFACE OR (NOT WIN32 AND NOT WIN64 AND NOT _WIN32 AND NOT _WIN64))
    ...
    macro(cwork_insert_resources cwork_path build_type)
        # windows: add resources to source files (if not header-only library)
        if(WIN32 OR WIN64 OR _WIN32 OR _WIN64)
            if(${CWORK_PROJECT_NAME}_SOURCE_FILES AND ${CWORK_PROJECT_NAME}_RESOURCE_DIRS)
                message("> including project resources...")
                foreach(_dir IN ITEMS ${${CWORK_PROJECT_NAME}_RESOURCE_DIRS})
        ...
        # linux/unix/bsd/mac/ios: copy resources near build target (if executable/shared-lib)
        # android: copy resources in app tree (if executable/shared-lib)
        elseif(${build_type} STREQUAL "dynamic" OR ${build_type} STREQUAL "executable" OR ${build_type} STREQUAL "console")
            if(${CWORK_PROJECT_NAME}_RESOURCE_DIRS)
