...
* *develop*: main development branch, with full continuous integration and code coverage. Small or safe features can be directly committed in *develop*. Complex/experimental commits belong in feature branches.
...
The *Version.cmake* and *CHANGELOG.md* files are automatically generated by the script '_scripts/update_changelog.sh' (or '_scripts/merge_develop_into_master.sh'), based on the prefixes of commits (see 'Commit guidelines' below).
To run './_scripts/update_changelog.sh' on Windows, the "Git bash" can be used.
...
> * **prefix**-library_or_module_name[issue_number]: description_of_commit

|    prefix    |             usage              |
|--------------|--------------------------------|
| *break-feat* | Feature that breaks backwards compatibility or major feature (for major release or new product line). Increases the major version of the project (ex: 1.#.#) and appears in change log. |
| *break-fix*  | Bugfix that breaks backwards compatibility. Increases the major version of the project (ex: 1.#.#) and appears in change log. |
| *break*      | Major release or new product line, without any feature or fix in the commit. Increases the major version of the project (ex: 1.#.#). |
| *feat*       | New feature or subfeature, or additional behavior. Increases the minor version of the project (ex: #.1.#) and appears in change log. If the same feature is subdivided in multiple commits, use *patch* for the first commits, then *feat* for the last commit (feature delivery). Neglectible changes made later (comments, formatting, ...) can also use *patch*. If a feature needs to be refactored or bug-fixed, use *fix* instead. |
| *fix*        | Bug fix or refactoring of a feature. Increases the fix version of the project (ex: #.#.1) and appears in change log. |
| *patch*      | Small changes that do not really affect the behavior of a feature, or comments or additional tests. No impact on version number. |
| *infra*      | Infrastructure changes: cmake, CI, coverage, scripts, ... No impact on version number. |
...
Examples:
> * **break-feat**-video: full support of vulkan API (exceptions removed)
> * **feat**-pattern: iterator/iterable pattern + declaration helpers
> * **fix**-pattern[9976]: iterator/iterable: fix missing pointer init
