...
// -- simple message box -- ----------------------------------------------------
  ...
  // note: reverse order of buttons (usual order on iOS)
  ...
  // note: restore order (reversed on iOS)
  ...
  MessageBox::Result MessageBox::show(const char* caption, const char* message, MessageBox::ActionType actions, 
    ...
    char* error = nullptr;
    uint32_t result = __showMessageBox_uikit(caption, message, __toNativeIcon(icon), const_cast<const char**>(&buttons[0]), length, 
                                             (void*)parent, &error);
    if (error) {
      try { 
